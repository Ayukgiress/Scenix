...
const VIDEO_FILTERS = [
  ...
  {
    name: "Vivid",
    style: "saturate(1.8) contrast(1.1)",
    description: "Enhanced colors",
  },
  {
    name: "Matte",
    style: "contrast(0.9) brightness(1.05) saturate(0.8)",
    description: "Soft, muted look",
  },
  {
    name: "B&W",
    style: "grayscale(1)",
  ...
  {
    name: "Warm",
    style: "sepia(0.4) saturate(1.3)",
    description: "Golden vintage feel",
  },
  {
    name: "Cool",
    style: "hue-rotate(30deg) saturate(1.2)",
    description: "Blue-toned cinematic",
  },
  {
    name: "Fade",
    style: "opacity(0.85) brightness(1.1) contrast(0.9)",
    description: "Dreamy fade",
  },
  {
    name: "Cinematic",
    style: "contrast(1.2) saturate(0.9) brightness(0.95)",
    description: "Hollywood style",
  },
  {
    name: "Vintage",
    style: "sepia(0.2) contrast(1.1) saturate(0.8)",
    description: "Old film look",
  },
  {
    name: "Drama",
    style: "contrast(1.4) saturate(0.7) brightness(0.9)",
    description: "High contrast mood",
  },
  {
    name: "Neon",
    style: "hue-rotate(180deg) saturate(1.5) brightness(1.1)",
    description: "Vibrant neon colors",
  },
  {
    name: "Noir",
    style: "grayscale(1) contrast(1.3) brightness(0.85)",
...
const VIDEO_TRANSITIONS = [
  {
    ...
    type: "fade-in",
    duration: 1.5,
    description: "Smooth fade from black",
  },
  {
    name: "Fade Out",
    type: "fade-out",
  ...
  },
  {
    ...
    type: "dissolve",
    duration: 1.2,
    description: "Classic dissolve effect",
  },
  {
    name: "Wipe Right",
    type: "wipe-right",
  ...
  },
  {
    ...
    type: "zoom-in",
    duration: 1.0,
    description: "Begin with zoom effect",
  },
  {
    name: "Zoom Out",
    type: "zoom-out",
    duration: 1.0,
    description: "End with zoom effect",
  },
];
...
const AUDIO_EFFECTS = [
  ...
  },
  {
    ...
    duration: 0,
    description: "Auto-adjust volume",
  },
  {
    name: "Echo",
    type: "audio-echo",
...
const VISUAL_EFFECTS = [
  ...
  },
  {
    ...
    type: "pixelate",
    style: "pixelate(8px)",
    description: "8-bit pixel effect",
  },
  {
    name: "Vignette",
    type: "vignette",
...
// Image overlays (professional overlays that work with blend modes)
const IMAGE_OVERLAYS = [
  { name: "Film Grain", type: "grain", description: "Analog film grain texture" },
  { name: "Light Leak", type: "lightleak", description: "Classic light leak effect" },
  { name: "Vignette", type: "vignette", description: "Darkened edges for focus" },
  { name: "Letterbox", type: "letterbox", description: "Cinematic black bars" },
  { name: "Lens Flare", type: "lensflare", description: "Camera lens flare effect" },
  { name: "Dust & Scratches", type: "dust", description: "Vintage film imperfections" },
];

// Shape overlays for custom graphics
const SHAPE_OVERLAYS = [
  { name: "Solid Rectangle", type: "rect", color: "#000000", description: "Black rectangle overlay" },
  { name: "Circle Mask", type: "circle", color: "#000000", description: "Circular frame" },
  { name: "Gradient Bar", type: "gradient", color: "linear", description: "Animated gradient overlay" },
];
...
export function EffectsPanel() {
  ...
  const addImageOverlay = (overlay: typeof IMAGE_OVERLAYS[0]) => {
    const clip: LocalClip = {
      id: `tmp_overlay_${crypto.randomUUID()}`,
      mediaId: "",
      type: "image",
      startTime: currentTime,
      duration: 999, // Persist across entire video by default
      track: 3, // Upper track to always be on top
      trimStart: 0,
      trimEnd: 999,
      volume: 0,
      metadata: { overlayType: overlay.type, label: overlay.name },
      transforms: { x: 50, y: 50, scale: 1, rotation: 0, opacity: 0.8 },
      blendMode: "overlay", // Default to overlay blend mode for perfect compositing
      effects: [],
    };
    addClipLocal(clip);
    if (accessToken) syncAddClip(clip, accessToken);
  ...
  const addShapeOverlay = (shape: typeof SHAPE_OVERLAYS[0]) => {
    const clip: LocalClip = {
      id: `tmp_shape_${crypto.randomUUID()}`,
      mediaId: "",
      type: "shape",
      startTime: currentTime,
      duration: 999, // Persist across entire video by default
      ...
      trimEnd: 999,
      volume: 0,
      metadata: { overlayType: shape.type, color: shape.color, label: shape.name },
      transforms: { x: 50, y: 50, scale: 1, rotation: 0, opacity: 1 },
      blendMode: "normal",
      effects: [],
    };
      ...
      <div className="grid grid-cols-2 gap-2">
        {VISUAL_EFFECTS.map((fx) => (
          <button
            key={fx.name}
            onClick={() =>
              addEffectToSelectedClip("filter", fx.name, { style: fx.style })
            }
        ...
        <div className="mt-4 pt-4 border-t border-border/60">
          ...
          <div className="space-y-2">
            {selectedClip.effects.map((effect) => (
              <div
                key={effect.id}
                className="flex items-center justify-between bg-card/50 rounded-md p-2"
              >
                <div>
                ...
                <button
                  onClick={() =>
                    removeEffectFromClip(selectedClip.id, effect.id)
                  }
  ...
  const renderStickers = () => (
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        <p className="text-[10px] font-medium uppercase tracking-wider text-muted-foreground">
          Image Overlays (Blend Mode: Overlay)
        </p>
        <div className="grid grid-cols-2 gap-2">
          {IMAGE_OVERLAYS.map((overlay) => (
            <button
              key={overlay.name}
              onClick={() => addImageOverlay(overlay)}
              title={`Add ${overlay.name} overlay`}
              className="flex flex-col items-center gap-1 rounded-lg border border-border/60 bg-card p-3 transition-all hover:border-primary/50 hover:bg-card/80"
            >
              <Layers className="size-6 text-primary mb-1" />
              <span className="text-[10px] font-medium text-center">{overlay.name}</span>
              <span className="text-[7px] text-muted-foreground text-center">{overlay.description}</span>
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          {SHAPE_OVERLAYS.map((shape) => (
            <button
              key={shape.name}
              onClick={() => addShapeOverlay(shape)}
              title={`Add ${shape.name} shape`}
              className="flex flex-col items-center gap-1 rounded-lg border border-border/60 bg-card p-3 transition-all hover:border-primary/50 hover:bg-card/80"
            >
              <Sparkles className="size-6 text-primary mb-1" />
              <span className="text-[10px] font-medium text-center">{shape.name}</span>
              <span className="text-[7px] text-muted-foreground text-center">{shape.description}</span>
            </button>
          ))}
        </div>
      </div>
